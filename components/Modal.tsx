...
const Modal:React.FC<ModalProps> = ({
  ...
  return (
    ...
    onOpenChange={onChange}>
        <Dialog.Portal>
            <Dialog.Overlay
            className="bg-neutrak-900/90 backdrop-blur-sm fixed inset-0">
                <Dialog.Content
                className="fixed drop-shadow-md border border-neutral-700 top-[50%] left-[50%] max-h-full h-full md:h-auto md:max-h-[85vh] w-full md:w-[90vw] md:max-w-[450px] translate-x-[-50%] translate-y-[-50%] rounded-md bg-neutral-800 p-[25px] focus:outline-none">
                    ...
                    <Dialog.Description
                    className="mb-5 tex-sm leading-normal text-center"
                    >
                        {description}
                    </Dialog.Description>
                    <div>
                    ...
                    <Dialog.Close asChild>
                        <button
                        className="text-neutral-400 hover:text-white absolute top-[10px] right-[10px] appearance-none items-center justify-center rounded-full focus:outline-none" >
                            <IoMdClose/>
                        </button>
                    </Dialog.Close>
                </Dialog.Content>
